...
\begingroup
\footnotesize
\setlength{\abovedisplayskip}{0.020in}
\setlength{\belowdisplayskip}{0.020in}
...
ATE admits a transition only if the approved step relation validates this full record:
\[
(s,o,a,T_\theta(a,s,t,\mathrm{rng}),s',o',r,d,\tau)
\]
where $s$ and $o$ denote the pre-step simulator state and learner observation; $a$ denotes the learner-submitted action; $T_\theta(a,s,t,\mathrm{rng})$ denotes the certified executed action after the measured actuator transform; $s'$ and $o'$ denote the post-step state and observation; $r$ denotes the reward computed from measured inputs; $d$ denotes termination/truncation; and $\tau$ denotes receipt metadata such as nonce, sequence, lane, and key epoch. $T_\theta$ covers legitimate clipping, scaling, action repeat, torque saturation, or safety projection. Exact mode replays deterministic CPU simulators. Receipt mode follows measured-evidence principles from remote attestation architectures~\cite{rats} and checks signatures, freshness, lane identity, and hash-chain continuity.
\endgroup
...
\textbf{Deployment note.} The strongest current evidence targets exact CPU replay and enrolled software roots; MuJoCo is the representative measured physics engine for the continuous-control rows~\cite{mujoco}. In remote or shared clusters, the same receipt interface can be bound to TPM-style remote integrity verification~\cite{riv}. Malicious approved closures remain an approval-policy failure, not an ATE admission failure.
...
\bibitem{mujoco} E. Todorov, T. Erez, and Y. Tassa. MuJoCo: A Physics Engine for Model-Based Control. IROS, 2012.
\bibitem{riv} G. C. Fedorkow, E. Voit, and J. Fitzgerald-McKay. RFC 9683: Remote Integrity Verification of Network Devices Containing Trusted Platform Modules. IETF, 2024.
\end{thebibliography}
