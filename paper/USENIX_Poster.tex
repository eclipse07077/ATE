...
\section*{Design}
\begin{center}
\includegraphics[width=0.93\linewidth]{2026-07-08-ATE-diagram-v2.pdf}\\[-0.35em]
\scriptsize \textbf{Figure 1.} ATE places the admission gate at the transition boundary, not at wrapper self-report. The monitor admits a learner update only when a trusted step relation or receipt path vouches for the submitted action, executed action, state/RNG, and learner-visible fields. This makes action-only logging insufficient for wrapper-level DAZE tampering.
\end{center}
\vspace{-0.12in}
...
\end{tabularx}
\vspace{-0.03in}
\scriptsize \textbf{Table 2.} Systems evidence for the admission path. Rows/records count validation checks or receipt records; failures count accepted violations or false rejects in the corresponding audit. These results test the software-root protocol, the MuJoCo promotion gate, stress cases, and batched GPU receipt handling.
\end{center}
\vspace{-0.07in}
The v3 protocol audit rejects rogue keys, closure/policy drift, stale nonces, key swaps, rollback, transition edits, stale/wrong-key receipts, lane mismatch, stale anchors, preflight violations, and transform-governance attacks. Batch receipts amortize cost: CartPole costs 11.18 $\mu$s/record at 1.20$\times$ with 7.0 bytes/record; MuJoCo overhead is 1.01--1.23$\times$; Brax/JAX GPU receipts cost 1.30--1.64$\times$ with 0.137 bytes/record.
